...
module.exports.modifyOCO = async function (json) {
  ...
  // Convert ocos array to JSON string for PostgreSQL JSONB column
  let ocosJson = JSON.stringify(ocos || []);
  let data = await query("UPDATE banks SET ocos = $2 WHERE bank_id = $1 RETURNING bank_id, ocos", [bank_id, ocosJson]);
  return data.rows;
};
