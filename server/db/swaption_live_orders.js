...
module.exports.insertSwaptionLiveOrders = async function (orders) {
...
module.exports.updateSwaptionLiveOrders = async function (orders) {
...
module.exports.deleteSwaptionLiveOrders = async function (order_ids, time) {
  try {
    if (time) {
      await query('UPDATE swaption_live_orders SET time_closed = ($2) WHERE order_id = ANY($1)', [order_ids, time]);
    } else {
      await query('UPDATE swaption_live_orders SET time_closed = NOW() WHERE order_id = ANY($1)', [order_ids]);
    }
  } catch (err) {
...
async function insert_order(order) {
  ...
  makeQueryArrays(order, a, f, v, ['order_id']);

  // Assemble the insert query string

  let qs;
  qs = 'INSERT INTO swaption_live_orders (';
  qs += f.join(',');
  qs += ') VALUES (';
  qs += v.join(',');
  qs += ') RETURNING *';
...
async function update_order(order) {
  ...
  let qs;
  qs = 'UPDATE swaption_live_orders SET (';
  qs += f.join(',');
  qs += ') = (';
  qs += v.join(',');
